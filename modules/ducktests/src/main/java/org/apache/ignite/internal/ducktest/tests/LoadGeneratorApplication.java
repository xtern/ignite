...
public class LoadGeneratorApplication extends IgniteAwareApplication {
    ...
    class LoadGenerator implements Runnable {
        ...
        public LoadGenerator(IgniteCache<Long, String> cache, long range, int warmup) {
            this.cache = cache;
            this.range = range;
            this.warmup = warmup;
        }
        ...
        @Override public void run() {
            ...
            res = new Result(TimeUnit.NANOSECONDS.toMillis(max), cnt - warmup, System.currentTimeMillis() - initTime);
        }
    ...
    @Override public void run(JsonNode jsonNode) {
        IgniteCache<Long, String> cache = ignite.getOrCreateCache(jsonNode.get("cacheName").asText());
        ...
        for (int i = 0; i < threadsCnt; i++) {
            LoadGenerator dataGen = new LoadGenerator(cache, range, warmup);
        ...
        recordResult("AVG_OPERATION", opsDurationSum / threadsCnt);
