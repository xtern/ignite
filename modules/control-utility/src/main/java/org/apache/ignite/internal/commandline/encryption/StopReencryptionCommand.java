...
public class StopReencryptionCommand implements Command<String> {
    ...
    @Override public Object execute(GridClientConfiguration clientCfg, Logger log) throws Exception {
        try (GridClient client = Command.startClient(clientCfg)) {
            Map<UUID, IgniteException> resErrs = executeTaskByNameOnNode(
                client,
                VisorStopReencryptionTask.class.getName(),
                CU.cacheId(argCacheGrpName),
                BROADCAST_UUID,
                clientCfg
            );

            for (Map.Entry<UUID, IgniteException> entry : resErrs.entrySet()) {
                boolean err = entry.getValue() != null;

                log.info(INDENT + entry.getKey() + ": reencryption has" + (err ? " not" : "") +
                    " been scheduled" + (err ? " (" + entry.getValue().getMessage() + ")," : "."));
            }
    ...
    @Override public String confirmationPrompt() {
        return "Warning: the command will change cache group enryption key. Node join during the key change " +
