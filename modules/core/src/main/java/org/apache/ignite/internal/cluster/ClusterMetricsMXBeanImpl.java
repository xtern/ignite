...
public class ClusterMetricsMXBeanImpl implements ClusterMetricsMXBean {
    ...
    /** Cluster metrics snapshot expire time. */
    ...
    private ClusterMetrics metrics() {
        ...
        if (expTime < U.currentTimeMillis()) {
            synchronized (mux) {
                if (expTime == expireTime) {
                    metrics = cluster.metrics();

                    expireTime = U.currentTimeMillis() + cluster.ignite().configuration().getMetricsUpdateFrequency();
                }
            }
        }
     ...
     */
    private ClusterGroup nodesGroup(boolean srv, boolean client) {
        if (srv && client) {
            ClusterGroup daemons = cluster.forDaemons();

            return daemons.nodes().size() > 0 ? cluster.forOthers(daemons) : cluster;
        } else if (srv)
