...
public class IgniteClusterSnapshoRestoreSelfTest extends AbstractSnapshotSelfTest {
    ...
    public void testBasicClusterSnapshotRestore() throws Exception {
        ...
        for (int i = 0; i < CACHE_KEYS_RANGE; i++)
            assertEquals(i, ignite.cache(dfltCacheCfg.getName()).get(i));

        for (int i = CACHE_KEYS_RANGE; i < CACHE_KEYS_RANGE * 2; i++)
            ignite.cache(dfltCacheCfg.getName()).put(i, i);
        ...
        System.out.println(">xxx> activating!");
        ...
        for (int i = CACHE_KEYS_RANGE; i < CACHE_KEYS_RANGE * 2; i++)
            assertNull(ignite.cache(dfltCacheCfg.getName()).get(i));
    }
    ...
    /** @throws Exception If fails. */
    @Test
    public void testClusterSnapshotRestore() throws Exception {
        int nodesCnt = 4;
        IgniteEx ignite = startGridsWithCache(nodesCnt - 1, dfltCacheCfg, CACHE_KEYS_RANGE);

        resetBaselineTopology();

        ignite.snapshot().createSnapshot(SNAPSHOT_NAME).get();

        Ignite newNode = startGrid(3);
        ...
        for (int i = 0; i < CACHE_KEYS_RANGE; i++)
            assertEquals(i, newNode.cache(dfltCacheCfg.getName()).get(i));

        for (int i = CACHE_KEYS_RANGE; i < CACHE_KEYS_RANGE * 2; i++)
            newNode.cache(dfltCacheCfg.getName()).put(i, i);
...
//        String igniteInstanceName = ignite.context().igniteInstanceName();

        StoredCacheData cacheData = readStoredCacheConfig(igniteCfg, SNAPSHOT_NAME, dfltCacheCfg.getName());

        assert cacheData != null;

        AffinityFunction aff = cacheData.config().getAffinity();

        AffinityTopologyVersion topVer = ignite.context().discovery().topologyVersionEx();

        System.out.println(">xxx> topVer=" + topVer);

        // todo server nodes?
        GridAffinityFunctionContextImpl ctx =
            new GridAffinityFunctionContextImpl(new ArrayList<>(ignite.cluster().nodes()), null, null, topVer, cacheData.config().getBackups());

        List<List<ClusterNode>> res = aff.assignPartitions(ctx);

        int p = 0;

        System.out.println(">xxx> parts=" + aff.partitions() + ", backups=" + cacheData.config().getBackups());

        for (List<ClusterNode> primaryBackup : res)
            System.out.println(">xxx> " + p++ + " " + F.viewReadOnly(primaryBackup, ClusterNode::id));

        Map<Integer, File> partsLocations = snapshotPartitionDistribution(SNAPSHOT_NAME, cacheData.config());

        assert partsLocations.size() == aff.partitions() : "required=" + aff.partitions() + ", found=" + partsLocations.size();

        for (Map.Entry<Integer, File> entry : partsLocations.entrySet())
            System.out.println(">xxx> " + entry.getKey() + " " + entry.getValue());

        for (Ignite g : G.allGrids()) {
            File cacheDir = resolveCacheDir(((IgniteEx)g).context().igniteInstanceName(), cacheData.config().getName());
            ...
            for (File file : cacheDir.listFiles(f -> f.getName().startsWith(FilePageStoreManager.PART_FILE_PREFIX))) {
        ...
        for (int i = 0; i < aff.partitions(); i++) {
            List<ClusterNode> list = res.get(i);

            File src = partsLocations.get(i);

            for (ClusterNode n : list) {
                File cacheDIr = resolveCacheDir(n.attribute(ATTR_IGNITE_INSTANCE_NAME), cacheData.config().getName());
                File target = new File(cacheDIr, src.getName());
        ...
        for (int i = CACHE_KEYS_RANGE; i < CACHE_KEYS_RANGE * 2; i++)
            assertNull(newNode.cache(dfltCacheCfg.getName()).get(i));
    }

    private Map<Integer, File> snapshotPartitionDistribution(String snpName, CacheConfiguration ccfg) {
        Map<Integer, File> res = new HashMap<>();

//        Set<Integer> reqParts = IntStream.range(0, ccfg.getAffinity().partitions()).boxed().collect(Collectors.toSet());

        for (Ignite g : G.allGrids()) {
            IgniteConfiguration cfg = g.configuration();

            File cacheDir = resolveSnapshotCacheDir(snpName, cfg, ccfg.getName());

            if (!cacheDir.exists())
                continue;

            for (String fileName : cacheDir.list()) {
                if (!fileName.startsWith(FilePageStoreManager.PART_FILE_PREFIX))
                    continue;

                int partId = Integer.parseInt(fileName.substring(FilePageStoreManager.PART_FILE_PREFIX.length(), fileName.indexOf('.')));

                if (res.containsKey(partId))
                    continue;

                res.put(partId, new File(cacheDir, fileName));
            }
        }
    ...
    private StoredCacheData readStoredCacheConfig(IgniteConfiguration cfg, String snpName, String cacheName) throws IOException, IgniteCheckedException {
        File cacheDir = resolveSnapshotCacheDir(snpName, cfg, cacheName);
    ...
    private File resolveSnapshotCacheDir(String snpName, IgniteConfiguration cfg, String cacheName) {
        File workDIr = IgniteSnapshotManager.resolveSnapshotWorkDirectory(cfg);
